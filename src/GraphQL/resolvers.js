const resolvers = {
    Query: {
        ...
        async categoryByName(parent, args, ctx, info){
            const isCategoryExists = await ctx.prisma.$exists.category({name: args.name})
        ...
        async productsByCategory(parent, args, ctx, info){
            const isCategoryExists = await ctx.prisma.$exists.category({id: args.categoryId})

            if(!isCategoryExists){
                throw new Error('Category not found')
            }

            const products = await ctx.prisma.category({id: args.categoryId}).products()
            return products
        }
    ...
    Product: {
        category(parent, args, ctx, info){ //category--> product.category
